...
### [log.csv]
- 경고창이 사라진 시점부터 log.csv파일에 사용자가 집중을 유지하는지를 체크하는 함수가 작동한다. 경고창이 사라진 시점이 log.csv파일의 1열 값인 lost_start에 입력된다. 그리고, 사용자의 동공 방향이 lost_start이후 CENTER를 본 시점이 lost_end에 입력되는 것이다. 따라서, 이 값은 프론트측에 넘어갔을 때 사용자가 집중을 유지한 것으로 간주된다. (사진 2)와 같이 
lost_start와 lost_end값의 차이가 0을 초과하는 경우는, 이 값이 프론트측에 넘어갔을 때 사용자가 집중을 잃었던 시간으로 간주되는 것이다.
