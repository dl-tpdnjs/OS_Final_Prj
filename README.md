...
4. 자바 에디터 속 **Main_All_test.java** 파일을 통해 어플리케이션을 실행시킨다.
    - 이때, 파일 경로들이 상대 경로로 설정되어 있어서 src 폴더의 바로 밖에 LectureLost.csv 파일이 위치한다면 대부분의 경우 올바르게 실행될 것이다.
    - 혹시라도 제대로 실행되지 않는다면, LectureLost.csv 파일에 대한 절대 경로를 상대 경로로 지정되어 있는 부분에 붙여넣어 실행하면 된다.
