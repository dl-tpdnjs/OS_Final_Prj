...
4. 경고창
   - 기존의 경고창은, 음성과 함께 "You have lost focus"라는 문구가 뜨도록 하였다. 여기서 opencv에서는 한국어를 지원하지 않고 있어, PIL을 이용하여 한국어로 "10초 내에 다시 영상 강의에 집중하세요!" 라는 문구가 뜨도록 수정하였다. 또한, 동공을 추적하는 웹캠 창에서도 현재 동공의 방향을 표시하도록 되어 있는데 여기서도 방향 표시 부분을 한국어로 문구를 수정하여 사용자가 조금 더 이해하기 쉽도록 개선하였다.
...
- **기능**
    - 경고창 개선
      - 카운트다운 표시, 경고 메세지 표시 시간 수정, 경고 이미지 삽입, 경고창 뜨는 동시에 경고 음성 재생되도록 수정
- **데이터**
    - log.csv 파일에 데이터 라벨별 삽입, 집중력 유지 시간 및 잃은 시간 조건 수정, 프로그램 실행 시 log.csv 파일 리셋 기능 추가
...
### [웹캠 화면 : 기본]
- 사용자의 동공 위치를 계산하여 동공 주위로 초록색의 원 테두리가 웹캠 화면에 그려진다. 사용자의 동공이 왼쪽을 향하면 '방향 : LEFT' 텍스트가, 정면을 향하면 '방향 : CENTER' 텍스트가, 오른쪽을 향하면 '방향 : RIGHT' 텍스트가 왼쪽 하단에 표시되도록 하여 사용자의 동공이 현재 어느 방향을 바라보고 있는지 알 수 있도록 해준다. 
- 사용자가 0.5초 이상 정면(화면에 표시되는 값: CENTER)이 아닌 왼쪽이나 오른쪽을 응시하거나 (화면에 표시되는 값: LEFT,RIGHT) , 화면에서 사라지는 경우 웹캠화면이었던 eye_tracker창이 경고화면으로 전환된다.

| 웹캡 화면 |
|----------|
![KakaoTalk_20240621_135857981_07](https://github.com/dl-tpdnjs/OS_Final_Prj/assets/167500948/5cb53926-8cf4-4a71-a888-6a304244df4f)
![KakaoTalk_20240621_135857981_06](https://github.com/dl-tpdnjs/OS_Final_Prj/assets/167500948/c14a36ce-cf09-42bc-9ead-d436377ad03e)
![KakaoTalk_20240621_135857981_05](https://github.com/dl-tpdnjs/OS_Final_Prj/assets/167500948/19b3e214-9cd9-49e0-b668-cd08a8f63981)
![KakaoTalk_20240621_135857981_03](https://github.com/dl-tpdnjs/OS_Final_Prj/assets/167500948/34f7356c-f2f7-423a-b463-c06b511b1002)
...
### [녹화 화면 : 경고]
- 경고화면은 새롭게 창이 뜨는 개념이 아닌, 기존 웹캠 화면이었던 eye_tracker화면에 warn()함수가 실행될 때 빨간색의 사각형 도형을 붙여넣고, 사용자의 주의 집중을 요하는 경고 이미지, “10초 내에 다시 영상 강의에 집중하세요!” 라는 경고 텍스트를 붙여넣어 경고 화면임을 알렸다. 경고화면이 표시됨과 동시에 경고 음성이 짧게 흘러나온다. 그리고 경고창 하단에는 다시 집중 상태를 유지하기까지 남은 시간 (경고창이 없어지기까지 남은 시간)이 10초부터 카운트다운 방식으로 표시되도록 하였다. 따라서 10초, 9초.. 1초까지 시간이 흐르면 경고창이 사라지고 웹캠 화면으로 다시 나타나게 된다. 

| 웹캠 경고 화면 |
|----------|
![KakaoTalk_20240621_135857981_02](https://github.com/dl-tpdnjs/OS_Final_Prj/assets/167500948/edc86cc5-ce92-41dc-b96e-93bc22ac1907)
![KakaoTalk_20240621_135857981_04](https://github.com/dl-tpdnjs/OS_Final_Prj/assets/167500948/68a0f140-84f4-4351-a645-9032a4e199a1)

### [log.csv]
- 경고창이 사라진 시점부터 log.csv파일에 사용자가 집중을 유지하는지를 체크하는 함수가 작동한다. 경고창이 사라진 시점이 log.csv파일의 1열 값인 lost_start에 입력된다. 그리고, 사용자의 동공 방향이 lost_start이후 CENTER를 본 시점이 lost_end에 입력되는 것이다. 따라서, 이 값은 프론트측에 넘어갔을 때 사용자가 집중을 유지한 것으로 간주된다. [사진1]의 1행과 같이 lost_start와 lost_end값의 차이가 0을 초과하는 경우는, 이 값이 프론트측에 넘어갔을 때 사용자가 집중을 잃었던 시간으로 간주되는 것이다.
- 프로그램을 재실행하면 함수에서 log.csv파일을 리셋하여 log.csv파일에 새롭게 값이 입력되도록 한다.

| 웹캠 경고 화면 |
|----------|
![KakaoTalk_20240621_135857981_01](https://github.com/dl-tpdnjs/OS_Final_Prj/assets/167500948/9da4aeb4-b393-4937-b852-e3899047ee5b)
[사진1]
![KakaoTalk_20240621_135857981](https://github.com/dl-tpdnjs/OS_Final_Prj/assets/167500948/79877e84-7117-469f-81a2-8f2751de58e4)
[사진2]
...
- util.py에서 사용자가 집중력을 잃은 시점을 csv 파일의 lost_start 부분에 입력하는 함수 추가
- util.py에서 기존의 txt파일에 입력되었던 내용을, csv파일에 입력이 되도록 수정하였고, 프로그램 재실행 시 리셋되는 부분 추가

![focus regain](https://github.com/dl-tpdnjs/OS_Final_Prj/assets/163426589/ff72eaf3-1ab8-4947-9244-321467e252bc)
- util.py에서 사용자가 집중력을 회복한 시점을 csv 파일의 lost_end 부분에 입력하는 함수 추가

![방향 한국어로 바꾸기](https://github.com/dl-tpdnjs/OS_Final_Prj/assets/163426589/65bc3c48-a7c8-409b-a92d-9e4ec4e70f47)
- eye_tracker.py에서 웹캠 화면의 방향을 나타내는 부분을 한국어로 수정

![집중 시간 측정](https://github.com/dl-tpdnjs/OS_Final_Prj/assets/163426589/10908a0f-eb2f-42e7-960f-e3df8b9f6b4a)
- eye_tracker.py에서 경고 메시지가 사라진 이후에 CENTER를 보는 시점을 측정하도록 수정

![image](https://github.com/dl-tpdnjs/OS_Final_Prj/assets/163426589/a63f2a84-dda8-435f-8b89-0e64e78dedb9)
- eye_tracker.py에서 사용자가 정면이 아닌 왼쪽이나 오른쪽 방향을 응시하거나, 화면에서 사라졌을 때 경고창이 뜨기까지의 시간을 2초 -> 0.5초로 수정
...
- 백엔드
    - 어두운 곳에서 프로그램을 실행하게 되거나, 사용자가 화면으로부터 거리가 조금 떨어져 있는 경우라면, 웹캠 화면에서 동공의 위치를 정확하게 잡아내지 못했다. 또한 동공이 정확히 어떤 위치를 응시하고 있는지 파악하려면 눈을 일정 크기 이상 뜨고 있어야 했다.
    - 왼쪽과 오른쪽, 정면에 대해서는 정확히 동공의 방향을 감지해주지만, 위나 아래를 보고 있었을 때, 눈을 감고 있을 때에도 계속 정면을 바라본다고 감지가 되기 때문에 정확한 집중도를 측정하기에 어려움이 있을 거라고 판단하였다.
    - 미세한 눈동자의 움직임만으로도 정면이 아닌 방향으로 감지가 되는 경우가 있었다. 이로 인해, 집중력을 잃었다고 판단하여 계속하여 경고창이 나타나게 되면, 오히려 사용자의 집중력을 더 흐트러트릴 가능성이 있다. 


    
<br>

## 9. 프로젝트 중 어려웠던 점 및 느낀 점
...
- 우선, 오픈소스를 가져다가 프로그램을 실행하기까지의 과정에서 어려움을 겪었다. 우리가 사용한 오픈소스는 파이썬, 텐서프로우 등 요구하는 버전이 현재 설정되어 있는 최신 버전들과 달랐기 때문에, 이에 맞게 하나하나 설정해줘야 해서 어려움을 겪었다.
    - 이 과정에서 강의록을 참고하여 아나콘다를 이용하여 가상환경을 만들고, 활성화하였으며, pip를 이용하여 필요한 패키지나 라이브러리를 설치하였고, 버전을 알맞게 설정하여 프로그램을 정상적으로 실행할 수 있었다.
- eye_tracker.py를 실행할 때 집중력을 잃은 시간을 어떤 기준으로 측정할 것인지가 관건이었다. 기존의 프로그램은 사용자가 일정 시간 이후에 다른 곳을 응시하면 경고 메시지가 뜨고, 경고 메시지가 뜬 시점을 집중력을 잃기 시작한 시간으로 측정한다. 하지만 경고 메시지가 사라지고 난 후에도 계속 정면이 아닌 왼쪽 또는 오른쪽을 응시하고 있었는데도 다시 경고창이 뜨지도 않았고, log.csv에 입력된 값들을 확인해보아도 집중을 잃은 시간에서부터 집중을 회복하기까지의 시간이 0초, 또는 1초 정도의 아주 간소한 차이로, 제대로 집중도 측정이 되지 않는다는 것을 파악했다. 집중도 측정에 문제가 생기면, 본래 목적이었던 사용자가 본인의 정확한 집중 정도를 파악하는 것이 어려울 수 있다. 따라서, 사용자의 집중력이 흐트러진 시간을 측정하는 방법은 여러 가지가 있겠지만 이중에서 어떤 방법을 기준으로 삼을지에 대해 어려움이 있었다.
    - 이 부분에 대해서는, 사용자의 집중 유지를 도와주는 프로젝트의 본래 목적을 살려 사용자에게 경고 메시지가 뜨는 10초 동안을 집중을 다시 유지할 수 있는 시간으로 남겨두고, 경고 메시지가 사라진 이후부터를 집중도로 측정하는 시간으로 코드를 개선하였다. 이후, 프로그램을 실행하는 동시에 네이버 서버 시간을 통해 확인해 본 결과, 경고 메시지가 사라진 후부터, 내가 CENTER를 보기까지의 시간이 집중력을 잃은 시간으로 정확하게 측정이 되는 것을 알 수 있었다. 
...
열정적이고 실력 있는 좋은 팀원 분들과 함께 팀 프로젝트를 진행할 수 있었어서 정말 감사했습니다. 덕분에 팀장인 저는 더욱 수월하게 제 본분을 다 할 수 있었습니다. 다들 개발 기간 동안 저와 함께 새벽을 보내주셔서 너무 감사했어요.
저희가 이번에 개발한 프로젝트는 최근 주목을 받고 있는 '조용한 ADHD'에 착안해 진행했는데, 제가 처음 그렸던 그림보다 더 퀄리티가 높은 작품이 나와서 정말 행복합니다.
오픈소스를 열고, 프로그램을 수정해보고 변형해보는 역할을 맡으며 프로그래밍에 더욱 재미가 붙은 것 같아요. 모두 저와 함께 해주신 팀원 분들 덕입니다.

잘 마무리 할 수 있도록 함께해주시고 도와주셔서 감사합니다!
...
### 👻 김주연
 ...
 이번 프로젝트에서는 팀원분들과의 꾸준한 연락을 통해 문제가 생긴 부분 해결이나, 프로젝트 진행에 필요한 의견 조율을 즉각적으로 진행할 수 있었습니다. 팀원 간의 적절한 소통으로 협업이 가능하다는 것을 다시 한 번 느꼈습니다. 이번 경험이 앞으로 팀 활동을 진행하는데 많은 도움이 될 것 같습니다. 사실 프로그램 제작이나 프로젝트 진행 경험이 많은 편이 아니라 처음에는 과연 잘 진행할 수 있을지 걱정이 많았는데, 저희 팀원분들 모두 열심히 해주시고 도움도 주신 덕분에 큰 문제 없이 끝까지 마칠 수 있었던 것 같습니다. 다들 정말 수고 많으셨습니다!
...
오픈소스 프로그래밍 수업에서 배운 아나콘다와 깃허브는 처음에는 낯설고, 이론으로만 이해할 때는 어떻게 다뤄야 할지 막막했습니다. 그러나 실제로 깃허브와 같은 오픈소스 커뮤니티를 활용해야 할 상황이 닥치자, 그 필요성을 절실히 느끼게 되었고 깃허브를 사용하기 전에는 수정되는 코드들을 문서에 저장하며 관리하는 데 어려움이 많았지만, 이제는 수정된 코드를 깃허브에 커밋함으로써 코드의 변화와 수정된 부분을 한눈에 파악할 수 있게 되었습니다. 

또한, 팀원들과 프로그램 실행에 필요한 파일들을 바로바로 공유할 수 있고, 이전 버전들까지 관리할 수 있어 협업에 있어 필수적인 도구임을 알게 되었습니다. 특히, 오픈소스 프로그래밍 수업의 특성에 맞게 공개된 소스 코드를 자유롭게 변형할 수 있다는 점이 흥미로웠습니다. 따라서, 오픈소스를 잘 활용하면 높은 퀄리티의 프로그램을 빠른 시간 내에 완성할 수 있다는 것을 이번 프로젝트를 통해 알게 되어, 이후에도 오픈소스를 활용한 프로그램을 제작해보려고 합니다. 

또한, 저희가 제작한 ADHD인들을 위한 온라인 강의 집중 보조 프로그램은 디지털 사회에서 온라인 강의의 비중이 커지는 현재, 사용성이 매우 높을 것이라고 생각합니다. 저 또한 실제로 집중력 산만으로 온라인 수업 집중에 어려움을 겪는 경우가 많았어서, 이번 프로젝트를 단순히 수업 과제로만 끝내는 것이 아니라, 실제로 온라인 강의를 들을 때 적극적으로 활용해보려고 합니다.
